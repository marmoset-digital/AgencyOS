...
/** Normalise a CSV header to a canonical field name */
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (contactEmail || contactFirstName || contactLastName) {
      if (!contactFirstName || !contactLastName || !contactEmail) {
        errors.push({
          row: rowNum,
          message: `${name}: Contact skipped — first name, last name, and email are all required for contact import`,
        })
      } else {
