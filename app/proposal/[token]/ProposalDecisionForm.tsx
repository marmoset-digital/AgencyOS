...
import { submitProposalDecision } from '@/app/actions/proposals'
...
export default function ProposalDecisionForm({ token }: { token: string }) {
  ...
  function submit(decision: 'accepted' | 'changes_requested') {
    setError(null)
    ...
    startTransition(async () => {
      const res = await submitProposalDecision(token, decision, name, comment)
  ...
  return (
    <div>
      <p className="text-sm text-gray-500 mb-4">Please review the proposal above and let us know how you’d like to proceed.</p>
      ...
      <input
        ...
        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#254DA5] mb-4"
      />
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        >
          {isPending ? 'Submitting…' : '✓ Accept proposal'}
        </button>
        <button
      ...
      <p className="text-xs text-gray-400 mt-4">By accepting, you confirm you’re authorised to sign off on behalf of your organisation.</p>
    </div>
  )
}
